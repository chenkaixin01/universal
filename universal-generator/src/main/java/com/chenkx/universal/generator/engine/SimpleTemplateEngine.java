package com.chenkx.universal.generator.engine;
 ...
 */
public class SimpleTemplateEngine extends UniversalTemplateEngine {

    @Override
    protected void generateJobs() {
        String path = ToolUtil.format(super.getContextConfig().getProjectPath() + getJobConfig().getJobPathTemplate(),
                super.getContextConfig().getBizEnName(),super.contextConfig.getBizEnName());
        generateFile(super.getContextConfig().getTemplatePrefixPath() + "/Job.java.btl", path);
        System.out.println("生成Job类成功!");
    }
