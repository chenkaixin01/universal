 ...
 */
	...
	$.jstree.plugins.checkbox = function (options, parent) {
		...
		this._undetermined = function () {
			...
			// attempt for client side undetermined state
 ...
 */
	...
	$.jstree.defaults.search = {
		/**
		 * a jQuery-like AJAX config, which jstree uses if a client should be queried for results.
		 *
		 * A `str` (which is the search string) parameter will be added with the request, an optional `inside` parameter will be added if the search is limited to a node id. The expected result is a JSON array with nodes that need to be opened so that matching nodes will be revealed.
		 * Leave this setting as `false` to not query the client. You can also set this to a function, which will be invoked in the instance's scope and receive 3 parameters - the search string, the callback to call with the array of nodes to load, and the optional node ID to limit the search to
	...
	$.jstree.plugins.search = function (options, parent) {
		 ...
		 * @param {Boolean} skip_async if set to true client will not be queried even if configured
